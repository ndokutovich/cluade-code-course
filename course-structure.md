...
| Режим | Разрешённые tools | Сценарий |
|-------|------------------|----------|
| Read-only | Read, Glob, Grep | Code review, аудит |
| Safe edit | Read, Glob, Grep, Edit | Редактирование без shell |
| Full access | Все | Полная разработка |
...
| Буфер обмена | `Ctrl+V` / `Cmd+V` | Вставка кода или изображения |
| Изображение | Вставка или drag-and-drop | Скриншот ошибки |
...
### 1.8.3 Буфер обмена и изображения

**Вставка текста:**
- `Ctrl+V` / `Cmd+V` — вставить текст из буфера
- Код автоматически форматируется

**Вставка изображений (отдельная команда!):**

| Платформа | Команда |
|-----------|---------|
| Windows | `Alt+V` |
| macOS | `Cmd+V` |
| Linux | `Ctrl+V` или `Alt+V` |

**Поддерживаемые изображения:**
- Скриншоты ошибок (терминал, браузер)
- Диаграммы и схемы
- UI макеты для реализации

**Примеры сценариев:**
```
[Alt+V — вставляете скриншот ошибки]
"Что означает эта ошибка и как её исправить?"

[Alt+V — вставляете скриншот UI]
"Реализуй этот дизайн в React"
```
